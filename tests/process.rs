...
process_test! {
    fn path_returns_correct_path(
        ...
        assert_eq!(path.components().last().unwrap().as_os_str().to_string_lossy().as_ref(), "test-program.exe");
        assert!(path.exists());
    }
}
...
process_test! {
    fn long_process_paths_are_supported(
        ...
        exe_path.push("test-program.exe");
...
#[test]
fn correct_bitness_on_self() {
    let current: BorrowedProcess = Process::current();
    let expected = if cfg!(target_pointer_width = "32") {
        32
    } else {
        64
    };
    let actual = current.bitness();
